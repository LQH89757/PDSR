...
class PDSRGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G', 'G_GAN', 'D_real', 'D_fake', 'idt', 'DVC', 'nll_A', 'nll_B', 'exp_A', 'exp_B', 'PCR', 'SCA']
    ...
    def compute_G_loss(self):
        ...
        self.loss_DVC = self.calculate_var_loss()

        if self.opt.lambda_NCE > 0.0:
            cam = pred_fake.detach()
            self.loss_SCA = self.calculate_BYOL_loss(self.feats, cam=cam) * self.opt.lambda_NCE
        else:
            self.loss_SCA = 0.0

        self.loss_G = self.loss_G_GAN + self.loss_idt + self.loss_SCA
        self.loss_PCR = self.criterionSSL(self.fake_B_rs, self.fake_B)

        if self.opt.lambda_var > 0:
            self.loss_G += self.opt.lambda_var * self.loss_DVC

        self.loss_G += self.loss_PCR
        return self.loss_G
