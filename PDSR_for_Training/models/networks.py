...
class PatchDensityEstimator(nn.Module):
    ...
    def create_flow(self, feats):
        ...
        for mlp_id, feat in enumerate(feats):
            ...
            if opt.flow_type == 'maf':
                ...
                print('[*] using MAFMOG')
                flow = MAFMOG(opt.flow_blocks, opt.maf_comps, input_nc, opt.maf_dim, opt.maf_layers,
                              input_order='random')
    ...
    def forward(self, feats, num_patches=256, patch_ids=None, detach=False, cams=None, prj=False, EnCo=False):
        ...
        if cams is not None or EnCo == True:
                ...
                if num_patches > 0:
                    ...
                    if patch_ids is not None:
                        ...
                        for b in range(cams[feat_id].shape[0]):
                            ...
                            points_sampled = torch.randperm(H * W, device=cam_b.device)[
                                             :num_points * self.oversample_ratio]
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if num_patches > 0:
                ...
                if isinstance(patch_id, torch.Tensor):
                    ...
                    patch_id = torch.tensor(patch_id, dtype=torch.long, device=feat.device)
                x_sample = feat_reshape[:, patch_id.clone().detach(), :].flatten(0, 1)  # reshape(-1, x.shape[1])
            else:
                x_sample = feat_reshape
                patch_id = []

            if feat_id > 0 and patch_ids is None:  # x_sample (256,128)
                x_fft = self.compute_fft_distribution(x_sample.clone().cpu().detach())
                x_uc = self.dirichlet_uncertainty(x_fft)
                x_uc = x_uc.tolist()
                if np.isnan(x_uc).any():
                    print("Nan in x_uc, contain all x_sample")
                else:
                    density_list = self.kmeans_threshold(x_uc)
                    t = patch_id.tolist()
                    filtered_patch_id = [t[i] for i in density_list]
    ...
    def select_patches(self, x_sample, density_list):

        indices = torch.tensor(density_list, dtype=torch.long)

        xr_sample = x_sample[indices]
        return xr_sample

    def compute_fft_distribution(self, x_sample, num_bins=10):

        num_patches, feature_dim = x_sample.shape  # (256, 256)

        fft_feat = torch.fft.fft(x_sample.to(dtype=torch.float32), norm="ortho")
        fft_mag = torch.abs(fft_feat)

        max_freq = feature_dim // 2
        bin_edges = torch.linspace(0, max_freq, num_bins + 1, device=x_sample.device, dtype=torch.float)

        freq_distribution = torch.zeros(num_patches, num_bins, device=x_sample.device)

        freqs = torch.arange(feature_dim, device=x_sample.device)
        radius = torch.abs(freqs - feature_dim // 2)

        for i in range(num_bins):
            mask = (radius >= bin_edges[i]) & (radius < bin_edges[i + 1])
            if mask.sum() > 0:
                freq_distribution[:, i] = fft_mag[:, mask].sum(dim=1)

        freq_distribution_sum = freq_distribution.sum(dim=1, keepdim=True)
        freq_distribution = torch.where(freq_distribution_sum > 0, freq_distribution / freq_distribution_sum,
                                        freq_distribution)

        return freq_distribution  # (256, num_bins)

    def dirichlet_uncertainty(self, freq_distribution):

        num_patches, num_bins = freq_distribution.shape

        alpha = freq_distribution / (freq_distribution.sum(dim=1, keepdim=True) + 1e-6) * 10 + 1e-6  # (256, num_bins)

        alpha0 = alpha.sum(dim=1)  # (256,)

        term1 = torch.lgamma(alpha0) - torch.lgamma(alpha).sum(dim=1)  # (256,)
        term2 = ((alpha - 1) * (torch.digamma(alpha) - torch.digamma(alpha0.unsqueeze(-1)))).sum(dim=1)  # (256,)

        entropy = term1 + term2  # (256,)
        uncertainty = torch.relu(entropy)

        return uncertainty  # (256,)

    def kmeans_threshold(self, probabilities):
...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input, layers=[], encode_only=False, nce_layers=[]):
        if -1 in layers:
            layers.append(len(self.model))
        if -1 in nce_layers:
            nce_layers.append(len(self.model))
